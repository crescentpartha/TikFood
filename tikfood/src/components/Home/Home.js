...
import DisplayItem from '../Dashboard/DisplayItem';
...
const Home = () => {
    ...
    let rest = [];
    const handleBooks = () => {
        for(let i=0; i<=7 && i<menus.length; i++) {
    ...
    return (
        <div className=''>
            ...
            {/* Welcome Section */}
            <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-5 px-10 items-center justify-center bg-base-200 py-20 rounded">
                ...
                <div className='px-5 md:mt-5'>
                    ...
                    <p className="py-6 font-bold text-3xl capitalize">Book your table <br /><span className='text-blue-500 font-semibold'><Link to='tel:+8801723242526'>+8801723242526</Link></span></p>
                </div>
            </div>

            {/* TikFood Menu */}
            <div className='px-10 py-24 bg-pink-100'>
                <h2 className='text-5xl mb-10 text-center uppercase font-semibold'>Our Food Menu</h2>
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-items-center text-left mb-4'>
                    {
                        rest.map(menu => <DisplayItem
                            key={menu._id}
                            menu={menu}
                        ></DisplayItem>)
                    }
                </div>
                <p className='text-blue-600 text-right italic'>
                    <Link to='/dashboard'>
                        <button className="btn">More Menu Items <FontAwesomeIcon icon={faArrowRight} className='ml-2' /></button>
                    </Link>
                </p>
            </div>
